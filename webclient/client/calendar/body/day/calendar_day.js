...
class CalendarDay extends HTMLElement {
    ...
    _update_display() {
        ...
        for (let i = 0; i < daily_subdivision; ++i) {
            let cell_time_start = new Date(this._date.getTime() + this.daily_start + i * this.daily_spacing);
            let cell_time_end = new Date(this._date.getTime() + this.daily_start + (i + 1) * this.daily_spacing);
    ...
    _update_events() {
        for (const event of this._elements[0].elements.events.children)
            event.remove();
        if (!this._even_pool)
            return;
    ...
    _add_event(sorted_event) {
        ...
        let vmin = Math.max(0, (event.start_time - this._date - this.daily_start) / this.day_duration());
        let vmax = Math.min(1, (event.end_time - this._date - this.daily_start) / this.day_duration());
